...
*   **Cloud Storage & History:** Access your generated mnemonics from anywhere and organize them by medical specialty.

### 3. Global Visual Challenge
A gamified review mode that continually tests your knowledge across all your saved mnemonics.
*   **Randomized Questions:** Draws from your entire library of generated content to keep knowledge fresh (spaced repetition style).
...
*   Google Cloud Project (optional, for persistent cloud storage)
...
3.  **Configure Environment:**
    Create a `secrets.toml` file in the root directory for local development (or use `.streamlit/secrets.toml`):
    ```toml
    [general]
    bucket_name = "your-bucket-name" # Optional: for GCS storage

    [gcp_service_account]
    # Optional: Google Cloud Service Account JSON for GCS
    type = "service_account"
    project_id = "..."
    # ... rest of your JSON key
    ```
    
    Ensure your `GEMINI_API_KEY` is set in your environment variables or `.env` file.
...
├── assets/                  # Static Assets for README etc.
├── data/                    # Data Storage
